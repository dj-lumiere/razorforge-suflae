...
function activate(context) {
    ...
    const serverPath = config.get('languageServer.path', 'RazorForge.exe');
    const serverArgs = config.get('languageServer.args', ['--lsp']);
    ...
    const clientOptions = {
        ...
        traceOutputChannel: vscode.window.createOutputChannel('RazorForge LSP Trace'),
    };
    try {
        // Create and start the language client
        client = new node_1.LanguageClient('razorforgeLanguageServer', 'RazorForge Language Server', serverOptions, clientOptions);
        ...
        vscode.window.showInformationMessage('RazorForge Language Support activated! LSP server starting...');
        console.log('RazorForge Language Support: Extension activated successfully');
    }
    catch (error) {
        console.error('RazorForge Language Support: Failed to start language server:', error);
        vscode.window.showErrorMessage(`Failed to start RazorForge Language Server: ${error}`);
    }
}
...
function registerCommands(context) {
    ...
    const restartCommand = vscode.commands.registerCommand('razorforge.restartLanguageServer', async () => {
        try {
            vscode.window.showInformationMessage('Restarting RazorForge Language Server...');
            if (client) {
                await client.stop();
                await client.start();
            }
            vscode.window.showInformationMessage('RazorForge Language Server restarted successfully');
        }
        catch (error) {
    ...
    const statusCommand = vscode.commands.registerCommand('razorforge.showServerStatus', () => {
        const isRunning = client?.isRunning() ?? false;
        const status = isRunning ? 'Running' : 'Stopped';
        vscode.window.showInformationMessage(`RazorForge Language Server Status: ${status}`);
    });
    // Command to create new RazorForge file
    const newFileCommand = vscode.commands.registerCommand('razorforge.newFile', async () => {
        try {
...
`;
            ...
            vscode.window.showErrorMessage(`Failed to create RazorForge file: ${error}`);
        }
    });
    // Command to create integrated project
    const createProjectCommand = vscode.commands.registerCommand('razorforge.createIntegratedProject', createIntegratedProject);
    // Formatting commands
    const formatDocumentCommand = vscode.commands.registerCommand('razorforge.formatDocument', () => formatRazorForgeDocument());
    const formatSelectionCommand = vscode.commands.registerCommand('razorforge.formatSelection', () => formatRazorForgeSelection());
    // Linting commands
    const lintDocumentCommand = vscode.commands.registerCommand('razorforge.lintDocument', () => lintRazorForgeDocument());
    const fixAllProblemsCommand = vscode.commands.registerCommand('razorforge.fixAllProblems', () => fixAllRazorForgeProblems());
    const organizeImportsCommand = vscode.commands.registerCommand('razorforge.organizeImports', () => organizeRazorForgeImports());
    context.subscriptions.push(restartCommand, statusCommand, newFileCommand, createProjectCommand, formatDocumentCommand, formatSelectionCommand, lintDocumentCommand, fixAllProblemsCommand, organizeImportsCommand);
}
/**
 ...
 */
function setupAutoSync(context) {
    ...
    const configWatcher = vscode.workspace.createFileSystemWatcher('**/razorforge.config.json');
    configWatcher.onDidChange(_uri => {
        vscode.window.showInformationMessage('RazorForge configuration changed. Restarting language server...');
        restartLanguageServer();
    });
    configWatcher.onDidCreate(_uri => {
        vscode.window.showInformationMessage('RazorForge configuration file created. Reloading...');
        restartLanguageServer();
    });
    // Watch for project file changes
    const projectWatcher = vscode.workspace.createFileSystemWatcher('**/*.{csproj,fsproj,vbproj}');
    projectWatcher.onDidChange(() => {
        vscode.window.showInformationMessage('Project file changed. RazorForge language server may need restart.');
    });
    ...
    const onWillSaveDocument = vscode.workspace.onWillSaveTextDocument(event => {
        if (event.document.languageId === 'razorforge') {
            ...
            if (formatOnSave) {
                const edit = vscode.commands.executeCommand('vscode.executeFormatDocumentProvider', event.document.uri);
                if (edit) {
    ...
    context.subscriptions.push(onDidChangeActiveTextEditor, onDidSaveDocument, configWatcher, projectWatcher, onWillSaveDocument);
}
/**
 * Checks for configuration changes that might affect RazorForge compilation.
 */
async function checkForConfigChanges(workspaceUri) {
    try {
        const configFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(workspaceUri, '**/razorforge.config.json'));
        const projectFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(workspaceUri, '**/*.{csproj,fsproj}'));
 ...
 */
function setupCrossExtensionIntegration(context) {
    const config = vscode.workspace.getConfiguration('razorforge');
    const suflaeIntegrationEnabled = config.get('suflaeIntegration', true);
    const sharedWorkspaceEnabled = config.get('sharedWorkspace', true);
    if (!suflaeIntegrationEnabled) {
        return;
    }
    // Check if Suflae extension is installed
    const suflaeExtension = vscode.extensions.getExtension('razorforge.sf-language-support');
    if (suflaeExtension) {
        ...
        vscode.window
            .showInformationMessage('RazorForge & Suflae integration activated! 🍰⚔️', 'Show Settings')
            .then(selection => {
            if (selection === 'Show Settings') {
                vscode.commands.executeCommand('workbench.action.openSettings', 'razorforge');
            }
        });
    }
    else {
        ...
        vscode.window
            .showInformationMessage('Install Suflae Language Support for complete RazorForge development experience?', 'Install', 'Not Now')
            .then(selection => {
            if (selection === 'Install') {
                vscode.commands.executeCommand('workbench.extensions.search', 'razorforge.sf-language-support');
            }
        });
    }
    // Watch for Suflae extension installation
    const extensionChangeHandler = vscode.extensions.onDidChange(() => {
        const newSuflaeExtension = vscode.extensions.getExtension('razorforge.sf-language-support');
        if (newSuflaeExtension && !suflaeExtension) {
            vscode.window.showInformationMessage('Suflae extension installed! Restart VS Code to enable full integration.');
        }
    });
...
function setupSharedConfiguration() {
    ...
    const razorForgeServerPath = workspaceConfig.get('razorforge.languageServer.path');
    const suflaeServerPath = workspaceConfig.get('suflae.languageServer.path');
    if (razorForgeServerPath && !suflaeServerPath) {
        workspaceConfig.update('suflae.languageServer.path', razorForgeServerPath, vscode.ConfigurationTarget.Workspace);
    }
    ...
    if (razorForgeFormatOnSave !== undefined && suflaeFormatOnSave === undefined) {
        workspaceConfig.update('suflae.formatOnSave', razorForgeFormatOnSave, vscode.ConfigurationTarget.Workspace);
    }
    console.log('RazorForge: Shared configuration synchronized with Suflae extension');
}
/**
...
RunTarget(target);`;
        ...
        await vscode.workspace.fs.writeFile(settingsPath, Buffer.from(JSON.stringify(settings, null, 2)));
        vscode.window
            .showInformationMessage('Integrated RazorForge + Suflae project created! 🍰⚔️', 'Open Main File')
            .then(selection => {
...
function registerFormattingProviders(context) {
    ...
    const documentFormattingProvider = vscode.languages.registerDocumentFormattingEditProvider('razorforge', {
        provideDocumentFormattingEdits(document) {
            return formatRazorForgeCode(document, null);
        },
    });
    // Range formatting provider
    const rangeFormattingProvider = vscode.languages.registerDocumentRangeFormattingEditProvider('razorforge', {
        provideDocumentRangeFormattingEdits(document, range) {
            return formatRazorForgeCode(document, range);
        },
    });
    // On-type formatting provider
    const onTypeFormattingProvider = vscode.languages.registerOnTypeFormattingEditProvider('razorforge', {
        provideOnTypeFormattingEdits(document, position, ch) {
            if (ch === '}' || ch === ';' || ch === '\n') {
                ...
                if (config.get('formatOnSave', true)) {
                    ...
                    const range = new vscode.Range(new vscode.Position(Math.max(0, position.line - 1), 0), position);
                    return formatRazorForgeCode(document, range);
                }
            }
            return [];
    ...
    context.subscriptions.push(documentFormattingProvider, rangeFormattingProvider, onTypeFormattingProvider);
}
/**
 * Registers linting and diagnostic features for RazorForge.
 */
function registerLintingFeatures(context) {
    const diagnosticsCollection = vscode.languages.createDiagnosticCollection('razorforge');
    context.subscriptions.push(diagnosticsCollection);
    // Code action provider for quick fixes
    const codeActionProvider = vscode.languages.registerCodeActionsProvider('razorforge', {
        provideCodeActions(document, range, context) {
            const actions = [];
            const config = vscode.workspace.getConfiguration('razorforge');
            // Auto-import actions
            if (config.get('codeActions.autoImport', true)) {
                const importAction = new vscode.CodeAction('Auto-import missing symbols', vscode.CodeActionKind.QuickFix);
            ...
            // Format action
            const formatAction = new vscode.CodeAction('Format code', vscode.CodeActionKind.SourceFixAll);
 ...
 */
function formatRazorForgeCode(document, range) {
    const config = vscode.workspace.getConfiguration('razorforge');
    const indentSize = config.get('formatting.indentSize', 4);
    const bracesOnNewLine = config.get('formatting.bracesOnNewLine', false);
    const trimTrailingWhitespace = config.get('formatting.trimTrailingWhitespace', true);
    const edits = [];
...
function updateDiagnostics(document, collection) {
    const diagnostics = [];
    const config = vscode.workspace.getConfiguration('razorforge');
    const minLevel = config.get('linting.level', 'warning');
    // Basic RazorForge linting rules
    for (let i = 0; i < document.lineCount; i++) {
        const line = document.lineAt(i);
        const text = line.text;
        // Check for trailing whitespace
        if (config.get('formatting.trimTrailingWhitespace', true)) {
            const trailingWhitespace = text.match(/\\s+$/);
            if (trailingWhitespace) {
                const start = new vscode.Position(i, text.length - trailingWhitespace[0].length);
                const end = new vscode.Position(i, text.length);
                const diagnostic = new vscode.Diagnostic(new vscode.Range(start, end), 'Trailing whitespace', vscode.DiagnosticSeverity.Hint);
        ...
        if (trimmed.includes('//') && !trimmed.startsWith('#')) {
            const diagnostic = new vscode.Diagnostic(line.range, 'RazorForge uses # for comments, not //', vscode.DiagnosticSeverity.Information);
            diagnostic.source = 'razorforge';
        ...
        if (trimmed.endsWith(';') && !trimmed.startsWith('#')) {
            const diagnostic = new vscode.Diagnostic(line.range, 'RazorForge does not require semicolons', vscode.DiagnosticSeverity.Hint);
            diagnostic.source = 'razorforge';
        ...
        if (leadingWhitespace && leadingWhitespace[0].includes('\\t')) {
            const diagnostic = new vscode.Diagnostic(new vscode.Range(i, 0, i, leadingWhitespace[0].length), 'Use spaces for indentation instead of tabs', vscode.DiagnosticSeverity.Information);
            diagnostic.source = 'razorforge';
 ...
 */
function createQuickFixActions(document, diagnostic) {
    const actions = [];
...
async function lintRazorForgeDocument() {
    const editor = vscode.window.activeTextEditor;
    if (editor && editor.document.languageId === 'razorforge') {
        const collection = vscode.languages.createDiagnosticCollection('razorforge');
