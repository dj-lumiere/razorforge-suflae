...
Generics in RazorForge are **partially working but complex** due to the multiple stages of compilation and the
interaction between different generic features. Here's what works, what doesn't, and why it's hard.
...
Until we can instantiate methods like `BackIndex<I>.resolve`, most stdlib code is broken. This is why `Console.show<T>`
being commented out is a symptom, not the root cause.
...
This is a **multi-month effort** for a production-ready implementation, but we can get a working prototype much faster
by focusing on Bug 12.13 first.
