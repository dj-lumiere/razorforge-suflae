...
// The word-division primitives (rf_udivrem_128_64 / rf_reciprocal_word /
// rf_udivrem_128_64_pre + the Möller-Granlund reciprocal) moved to rf_divide.c so that
// translation unit is fully self-contained and can be compiled straight to LLVM bitcode
// for the dev-loop LTO step (bignum_functions.c pulls in libtommath, which the bitcode
// step must not). Both files are linked into razorforge_runtime; the divide symbols are
// exported from rf_divide.c.
