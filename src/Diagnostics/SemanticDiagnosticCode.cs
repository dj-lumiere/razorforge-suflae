...
public enum SemanticDiagnosticCode
{
    ...
    /// via <c>==</c>, <c>cmp</c>, <c>hash</c>) to a buildtime member value (<c>me.$nameof(m)</c>) without
    ...
    /// <summary>A buildtime <c>expand</c> statement is nested inside another <c>expand</c>. Expansion is
