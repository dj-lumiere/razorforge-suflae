...
namespace RazorForge.LanguageServer
{
    ...
    public class RazorForgeLSP
    {
        ...
        {
            _logger.LogInformation("Starting RazorForge Language Server...");

            try
            {
                _server = await OmniSharp.Extensions.LanguageServer.Server.LanguageServer.From(options =>
                    options
            ...
            {
                _logger.LogError(ex, "Language Server failed to start");
                return 1;
            }
        }
        ...
        private void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddSingleton<SemanticAnalyzer>(provider =>
                new SemanticAnalyzer(Language.RazorForge, LanguageMode.Normal));
        ...
        private Task OnInitialize(ILanguageServer server, InitializeParams request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Language Server initializing...");
        ...
        /// <param name="cancellationToken">Cancellation token for the operation</param>
        /// <returns>Completed task</returns>
        private Task OnInitialized(ILanguageServer server, InitializedParams request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Language Server initialized successfully");
            return Task.CompletedTask;
        }
        ...
        {
            _logger.LogInformation("Language Server started and ready for connections");
            return Task.CompletedTask;
        }
        ...
        public async Task StopAsync()
        {
            if (_server != null)
            {
                _logger.LogInformation("Stopping RazorForge Language Server...");
                _server.Dispose();
            }
        }
    }
    ...
    public static class LanguageServerProgram
    {
        ...
        public static async Task<int> Main(string[] args)
        {
            ...
            if (args.Length > 0 && args[0] == "--lsp")
            {
                ...
                var logger = loggerFactory.CreateLogger<RazorForgeLSP>();
                var lsp = new RazorForgeLSP(logger);

                return await lsp.StartAsync(args);
            }
