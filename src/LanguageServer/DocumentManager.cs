...
public class DocumentManager
{
    ...
    public DocumentManager(ILogger<DocumentManager> logger,
        IRazorForgeCompilerService compilerService)
    {
        _logger = logger;
        ...
        string text)
    {
        ...
        AnalyzeDocument(document: document);
    }
    ...
    public void ChangeDocument(string uri, int version,
        IEnumerable<TextDocumentContentChangeEvent> changes)
    {
        if (!_documents.TryGetValue(key: uri, value: out DocumentState? document))
        {
            _logger.LogWarning(message: $"Attempted to change unknown document: {uri}");
            return;
        }

        _logger.LogDebug(message: $"Document changed: {uri}, version: {version}");

        // Apply incremental changes
        string newText = ApplyChanges(originalText: document.Text, changes: changes);

        DocumentState updatedDocument = document with
        {
        ...
        // Trigger re-analysis
        AnalyzeDocument(document: updatedDocument);
    }
    ...
    {
        _logger.LogInformation(message: $"Closing document: {uri}");
        _documents.TryRemove(key: uri, value: out _);
    }
    ...
    {
        return _documents.TryGetValue(key: uri, value: out DocumentState? document)
            ? document
            : null;
    }
    ...
    private void AnalyzeDocument(DocumentState document)
    {
        try
        {
            _logger.LogDebug(message: $"Analyzing document: {document.Uri}");

            // Compile and analyze the document
            CompilationResult result =
                _compilerService.AnalyzeCode(code: document.Text, filePath: document.Uri);
            ...
            _logger.LogDebug(
                message: $"Analysis complete: {document.Uri}, errors: {result.Errors.Count}");
        }
        catch (Exception ex)
        {
            _logger.LogError(exception: ex,
                message: $"Failed to analyze document: {document.Uri}");
        }
    }
    ...
    private string ApplyChanges(string originalText,
        IEnumerable<TextDocumentContentChangeEvent> changes)
    {
        ...
        foreach (TextDocumentContentChangeEvent change in changes)
        {
            ...
            else
            {
                ...
                {
                    string beforeChange = startLine < lines.Length
                        ? lines[startLine]
                           .Substring(startIndex: 0,
                                length: Math.Min(val1: startChar, val2: lines[startLine].Length))
                        : "";
                    string afterChange = endLine < lines.Length
                        ? lines[endLine]
                           .Substring(startIndex: Math.Min(val1: endChar,
                                val2: lines[endLine].Length))
                        : "";

                    string changedText = beforeChange + change.Text + afterChange;
                    string[] changedLines = changedText.Split(separator: '\n');

                    newLines.AddRange(collection: changedLines);
                }
                else
                {
                    // Change is beyond current text
                    newLines.Add(item: change.Text);
                }
