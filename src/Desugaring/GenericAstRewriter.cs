...
internal static class GenericAstRewriter
{
        ...
        TypeRegistry? registry)
    {
        ...
        public TypeInfo? ResolveType(TypeInfo? original)
        {
            ...
            // WrapperTypeInfo (Snatched[T] → Snatched[S64], or Snatched[S64] → stays): always
            // resolve to the real RecordTypeInfo so LLVM mangled names use "Core.Snatched[S64]"
            // (from RecordTypeInfo.FullName) rather than "Snatched[Core.S64]" (WrapperTypeInfo
            // with Module=null). The WrapperTypeInfo.FullName appends inner.FullName which includes
            // the module prefix, producing the wrong "Snatched[Core.S64]" format.
        ...
        RewriteContext ctx)
    {
        Expression result = expr switch
        {
            ...
            CallExpression { Callee: MemberExpression { PropertyName: var bsName } bsCallee,
                ...
                when ctx.Registry != null && Passes.BuilderServiceInliningPass.IsFoldable(bsName)
        ...
        MemberExpression callee, SourceLocation location, RewriteContext ctx)
    {
        ...
        return callee.PropertyName switch
        {
            "data_size" when u64Type != null && byteSizeType != null =>
                Passes.BuilderServiceInliningPass.MakeByteSizeCreatorPublic(
                    Passes.BuilderServiceInliningPass.CalculateDataSizeForType(typeInfo),
