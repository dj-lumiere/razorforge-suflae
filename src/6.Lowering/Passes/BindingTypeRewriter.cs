...
internal sealed class BindingTypeRewriter : ISyntaxTreeVisitor<bool>
{
    ...
    {
        if (node.Name == _bindingName)
        {
            node.ResolvedType = _concreteType;
        }
    ...
    public bool VisitCallExpression(CallExpression node)
    {
            ...
            id.Name == _bindingName)
        {
            ...
            RoutineInfo? resolved = _registry.LookupMemberRoutine(type: _concreteType,
                memberRoutineName: me.MemberName,
    ...
    {
        foreach ((Expression Key, Expression Value) p in node.Pairs)
        {
            Visit(e: p.Key);
            Visit(e: p.Value);
        }
    ...
    {
        foreach ((string Name, Expression Value) mv in node.MemberVariables)
        {
            Visit(e: mv.Value);
        }
    ...
    public bool VisitConditionalExpression(ConditionalExpression node)
    {
        ...
        Visit(e: node.TrueExpression);
        Visit(e: node.FalseExpression);
        return false;
    }
    ...
    {
        foreach (ExpressionPart p in node.Parts.OfType<ExpressionPart>())
        {
            Visit(e: p.Expression);
        }
    ...
    {
        Visit(e: node.Condition);
        Visit(s: node.ThenStatement);
        Visit(s: node.ElseStatement);
        return false;
    }
    ...
    public bool VisitWhenStatement(WhenStatement node)
    {
        ...
        if (node.ArmExpansion != null)
        {
            Visit(s: node.ArmExpansion.Template.Body);
        }
    ...
    {
        Visit(e: node.Resource);
        Visit(s: node.Body);
        if (node.FallbackBody != null)
        {
            Visit(s: node.FallbackBody);
        }
