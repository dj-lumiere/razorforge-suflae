...
// MonomorphizedBody record is defined in src/Instantiation/MonomorphizedBody.cs
...
internal sealed class MonomorphizationPlanner
{
    ...
    /// <summary>
    /// Pre-rewritten bodies produced by <see cref="Compiler.Instantiation.Passes.GenericMonomorphizationPass"/>
    /// at synthesis time, keyed by the concrete <see cref="TypeModel.Symbols.RoutineInfo.RegistryKey"/>.
        ...
        TypeInfo resolvedOwnerType, Dictionary<string, TypeInfo>? methodTypeArgs = null)
    {
        ...
        // Case 3: Generic-type owner (e.g. List[S64].add_last, Hijacked[T].$eq)
        TypeInfo? genericDef = resolvedOwnerType switch
        {
            ...
            // WrapperTypeInfo (Hijacked, Retained, etc.) stores the inner type in TypeArguments
            // but has no GenericDefinition property.
            // After SubstituteMethodForOwner, genericMethod.OwnerType may be the concrete type
            // (not the generic def), so also look up the generic def by wrapper name.
            WrapperTypeInfo wt when wt.TypeArguments is { Count: > 0 } =>
                genericMethod.OwnerType is { IsGenericDefinition: true }
                    ? genericMethod.OwnerType
                    : _registry.LookupType(name: wt.Name) is { IsGenericDefinition: true } td
                        ? td
                        : null,
            _ => null
        };
    ...
    public void PreRewriteAll(IReadOnlyDictionary<string, Statement> synthesizedBodies)
    {
        ...
        // Fold any residual BuilderService constant calls (e.g. Byte.data_size() left after
        // GenericAstRewriter substituted T ??"Byte" but couldn't resolve "Byte" at rewrite time).
        // GenericMonomorphizationPass bodies in PreMonomorphizedBodies are already folded by
        // BuilderServiceInliningPass.RunOnPreMonomorphizedBodies(); this sweep covers the bodies
        // built by the slow-path FindAstRoutine route above.
        new BuilderServiceInliningPass(_registry).RunOnMonomorphizedBodies(MonomorphizedBodies);
    }
        ...
        if (astRoutine == null)
        {
            ...
            if (entry.GenericMethod.IsSynthesized)
            {
                ...
                TypeInfo? ownerGenDef = entry.ResolvedOwnerType switch
                {
                    EntityTypeInfo { GenericDefinition: not null } e => e.GenericDefinition,
                    RecordTypeInfo { GenericDefinition: not null } r => r.GenericDefinition,
                    // WrapperTypeInfo (Hijacked[T], Retained[T], etc.) ??look up generic def by name
                    WrapperTypeInfo wt => _registry.LookupType(name: wt.Name),
                    _ => null
                };
                if (ownerGenDef != null)
                {
    ...
    public TypeInfo ResolveSubstitutedType(TypeInfo type, Dictionary<string, TypeInfo> subs)
    {
        ...
        // WrapperTypeInfo (Retained[T], Hijacked[T], etc.) never has IsGenericResolution=true.
        // Resolve by substituting the InnerType, then look up the RecordTypeInfo definition.
        // Handles both unresolved (InnerType=T) and pre-substituted (InnerType=Node) cases.
        if (type is WrapperTypeInfo wt)
        {
            TypeInfo resolvedInner = ResolveSubstitutedType(type: wt.InnerType, subs: subs);
