...
public sealed partial class TypeRegistry
{
    ...
    private static void RegisterAsAmbient(TypeRegistry registry)
    {
        _ambient = registry;
    }
    ...
    /// <summary>
    /// Stores the live-type set computed by TypeLivenessPass.  Called once, before Phase 6 synthesis.
    /// </summary>
    public void SetLiveConcreteTypes(HashSet<string> liveTypes)
    {
    ...
    private void MaterializeIfLazy(TypeSymbol type)
    {
        ...
        bool isSelfNesting = type.TypeArguments != null &&
                             type.TypeArguments.Any(predicate: arg =>
                                 arg.FullName.Contains(value: bareBaseName));
        if (!isSelfNesting)
        {
            _gmpDiscoveryQueue.Enqueue(item: type);
        }
    }
    ...
    /// </summary>
    public void UseModuleResolver(ModuleResolver resolver)
    {
    ...
    private readonly Dictionary<string, Dictionary<string, List<RoutineInfo>>> _routinesByOwner =
        new();
    ...
    private readonly HashSet<string> _optInDeriveMemberRoutines =
    ...
    public void MarkOptInDeriveMemberRoutine(string memberRoutine)
    {
        _optInDeriveMemberRoutines.Add(item: memberRoutine);
    }

    /// <summary>True if a derive memberRoutine name was marked opt-in via a capability gate.</summary>
    public bool IsOptInDeriveMemberRoutine(string memberRoutine)
    {
        return _optInDeriveMemberRoutines.Contains(item: memberRoutine);
    }
    ...
    private readonly Dictionary<string,
        List<(string OwnerParam, int Arity, List<GenericConstraintDeclaration> Gates, Statement
            Body)>> _deriveTemplates = new();
    ...
    public void RestoreStdlibPrograms(
    ...
    {
        return module != null &&
               _moduleDependencies.TryGetValue(key: module, value: out List<string>? deps)
            ? deps
            : [];
    }
    ...
    {
        return _moduleResolver?.EnumerateSubmodulePaths(prefix: prefix) ?? [];
    }
        ...
        out string? effectiveModule)
    {
        ...
        bool isBareModuleName =
            !importPath.Contains(value: '/') && !importPath.Contains(value: '.');
        ...
        return LoadResolvedModuleFile(resolvedPath: resolvedPath,
            moduleId: moduleId,
            effectiveModule: out effectiveModule);
    }

    /// <summary>
    /// Loads a module from an already-resolved file path: dedups by RESOLVED FILE PATH (serving an
    /// existing registration when the file was already loaded under a different import-path alias, to
    /// avoid duplicate-definition errors), otherwise marks it loaded (before parsing, to break import
    /// recursion) and loads it via <see cref="StdlibLoader"/>.
    /// </summary>
    private bool LoadResolvedModuleFile(string resolvedPath, string moduleId,
        out string? effectiveModule)
    {
        ...
        if (_loadedFilePaths.TryGetValue(key: fullResolvedPath,
                value: out string? alreadyLoadedModule))
        {
            _loadedModules.Add(item: moduleId);
    ...
    /// </summary>
    public static string RealmRoutineKey(RoutineInfo routine)
    {
        return routine.RegistryKey;
    }
    ...
    private TypeSymbol? LookupTypeInAmbient(string name)
    {
        ...
        if (_resolutions.TryGetValue(key: name, value: out TypeSymbol? resolution))
        {
            ...
            {
                MaterializeIfLazy(type: resolution);
            }
    ...
    public TypeSymbol? ReResolveInRealm(TypeSymbol type, string realm)
    {
        ...
        if (genericDef != null && type.TypeArguments is { Count: > 0 } args)
        {
            ...
            return tDef != null
                ? GetOrCreateResolution(genericDef: tDef, typeArguments: [.. args])
                : null;
        }
    ...
    public TypeSymbol GetOrCreateResolution(TypeSymbol genericDef, List<TypeSymbol> typeArguments)
    {
        ...
        if (TryGetCachedResolution(fullKey: fullKey,
                shortKey: shortKey,
                moduleFullKey: moduleFullKey,
                genericDef: genericDef,
                typeArguments: typeArguments) is { } cachedHit)
        {
            return cachedHit;
        }
        ...
        {
            EnqueueDiscoveredResolution(resolved: resolved);
        }
    ...
    /// <summary>
    /// Notifies GMP's fixed-point loop about a newly created concrete entity/record resolution. Guards:
    /// (1) fully-concrete (no unresolved param args, avoiding LookupMemberRoutine recursion); (2) no
    ...
    private void EnqueueDiscoveredResolution(TypeSymbol resolved)
    {
            ...
            IsFullyConcrete(t: resolved))
        {
            ...
            bool isSelfNesting = resolved.TypeArguments != null &&
                                 resolved.TypeArguments.Any(predicate: arg =>
                                     arg.FullName.Contains(value: bareBaseName));
            if (!isSelfNesting)
            {
                _gmpDiscoveryQueue.Enqueue(item: resolved);
            }
        }
    }

    /// <summary>
    /// Probes the three resolution-cache aliases (module-qualified, bare full, bare short) for an
    /// existing instance of the requested generic, materializing it if lazy. The module-qualified key is
    /// distinct across modules/realms so it is checked first; the bare aliases are module-blind and only
        ...
        string? moduleFullKey, TypeSymbol genericDef, List<TypeSymbol> typeArguments)
    {
            ...
            _resolutions.TryGetValue(key: moduleFullKey, value: out TypeSymbol? existing))
        {
            if (!_stdlibAnalysisActive)
            {
                MaterializeIfLazy(type: existing);
            }
            ...
            foreach (MemberVariableInfo field in sourceMembers)
            {
                ...
                target.Add(
                    item: new MemberVariableInfo(name: template.NamePrefix + field.Name,
                        type: columnType)
                    {
                        Visibility = template.Visibility, Index = target.Count, Owner = resolved
                    });
            }
        }
    }
    ...
    private static bool ResolutionTypeArgsMatch(TypeSymbol resolved, List<TypeSymbol> typeArguments)
    {
        ...
        {
            if (actual[index: i].FullName != typeArguments[index: i].FullName)
            {
                return false;
            }
        }
        ...
        ProtocolTypeSymbol genericDef)
    {
        ...
        {
            ProtocolMemberRoutineInfo? instMemberRoutine =
                instance.MemberRoutines.FirstOrDefault(predicate: m =>
                    m.Name == defMemberRoutine.Name &&
                    m.IsFailable == defMemberRoutine.IsFailable);
            if (instMemberRoutine == null || instMemberRoutine.ParameterTypes.Count !=
                defMemberRoutine.ParameterTypes.Count)
            {
                return true;
            }
        }
    ...
    /// FreeRoutine types are cached by their signature.
    /// </summary>
        ...
        TypeSymbol? returnType, bool isFailable = false)
    {
        ...
        // A routine VALUE is a trivially-destructible LEAF (a fat pointer over non-owned code + env), but the
        // auto-derived record/entity `destroy` field-walk (`me.$nameof(m).destroy()`, DeriveText) assumes EVERY
        // member type has a `destroy` — "a no-op for a raw-pointer / scalar / borrow leaf". A routine-typed
        // field otherwise resolves that call to nothing, so codegen mangles + references `Routine[...].destroy`
        // with no definition (link-undefined). Register the no-op leaf destroy here (BODY = bare `return`,
        // synthesized in WiredRoutinePass's routine-type loop) so the invariant holds and the field-walk links.
        RegisterRoutine(routine: new RoutineInfo(name: "destroy")
        {
            Kind = RoutineKind.MemberRoutine,
            OwnerType = newType,
            Parameters = [],
            ReturnType = LookupType(name: "None"),
            IsFailable = false,
            DeclaredMutation = MutationCategory.Readonly,
            MutationCategory = MutationCategory.Readonly,
            Visibility = VisibilityModifier.Open,
    ...
    public TupleTypeSymbol GetOrCreateTupleType(List<TypeSymbol> elementTypes)
    {
        ...
        RegisterTupleEqualityRoutines(newType: newType, elementTypes: elementTypes);
        RegisterTupleHashRoutine(newType: newType, elementTypes: elementTypes);
        RegisterTupleSerializeRoutine(newType: newType, elementTypes: elementTypes);
        RegisterTupleStoreRoutine(newType: newType, elementTypes: elementTypes);
        RegisterTupleComparisonRoutines(newType: newType, elementTypes: elementTypes);
    ...
    private void RegisterTupleDisplayRoutines(TupleTypeSymbol newType)
    {
        ...
        if (textType != null)
        {
            RegisterSynthesizedMemberRoutine(name: RuntimeContract.Display.Represent,
                owner: newType,
                returnType: textType);
            RegisterSynthesizedMemberRoutine(name: RuntimeContract.Display.Diagnose,
                owner: newType,
                returnType: textType);
        }
    }
    ...
    /// <summary>
    /// Auto-registers <c>eq</c> and <c>ne</c> if every element type has <c>eq</c> (option a: structural
    ...
    private void RegisterTupleEqualityRoutines(TupleTypeSymbol newType, List<TypeSymbol> elementTypes)
    {
                ...
                LookupMemberRoutine(type: et, memberRoutineName: "eq") != null))
        {
            ...
            RegisterSynthesizedMemberRoutine(name: "eq",
                ...
                returnType: boolType,
                parameters: [youParam]);
            RegisterSynthesizedMemberRoutine(name: "ne",
                owner: newType,
                returnType: boolType,
                parameters: [youParam]);
        }
    ...
    private void RegisterTupleHashRoutine(TupleTypeSymbol newType, List<TypeSymbol> elementTypes)
    {
        ...
        {
            RegisterSynthesizedMemberRoutine(name: "hash", owner: newType, returnType: u64Type);
        }
    }

    /// <summary>
    /// Auto-registers <c>serialize</c> if EVERY element is serializable — or a routine (the derive
    /// template boxes a routine element's signature via <c>represent</c>). A tuple can hold otherwise-
                ...
                et is not GenericParameterTypeSymbol && (et is RoutineTypeSymbol ||
                                                       LookupMemberRoutine(type: et,
                                                           memberRoutineName: RuntimeContract
                                                              .Serialize) != null)))
        {
            RegisterSynthesizedMemberRoutine(name: RuntimeContract.Serialize,
                owner: newType,
    ...
    /// <summary>
    /// Auto-registers <c>store</c> (the retaining field-walk copy) iff every element is itself Assignable
    /// — a value / managed leaf / RC wrapper, never a bare <c>entity</c> or access token. Without it,
    /// <c>var u = t</c> on a tuple holding a managed leaf (Text) bitwise-aliases and double-frees at the
    ...
    private void RegisterTupleStoreRoutine(TupleTypeSymbol newType, List<TypeSymbol> elementTypes)
    {
        ...
        {
            RegisterSynthesizedMemberRoutine(name: "assign", owner: newType, returnType: newType);
        }
    }
        ...
        List<TypeSymbol> elementTypes)
    {
                ...
                LookupMemberRoutine(type: et, memberRoutineName: "cmp") != null))
        {
            ...
            {
                RegisterSynthesizedMemberRoutine(name: opName,
                    owner: newType,
                    returnType: boolType,
                    parameters: [youParam]);
            }
    ...
    public List<TypeSymbol> GetProtocolImplementors(ProtocolTypeSymbol protocol)
    {
        ...
        foreach (TypeSymbol type in candidates)
        {
            ...
            TryAddImplementor(protocol: protocol,
                protocolBaseName: protocolBaseName,
                type: type,
        ...
        TypeSymbol impl)
    {
        ...
        {
            return ProtocolTypeArgsMatch(protocol: protocol, impl: impl)
                ? type
                : null;
        }
    ...
    private static bool ProtocolTypeArgsMatch(ProtocolTypeSymbol protocol, TypeSymbol impl)
    {
        ...
        {
            if (protocol.TypeArguments[index: i].FullName != impl.TypeArguments[index: i].FullName)
            {
                return false;
            }
        }
    ...
    /// <summary>
    /// Attempts to synthesize a concrete obeyer instance of a GENERIC implementing <paramref name="type"/>
    /// by binding each of its own generic parameters from the queried protocol's concrete arguments (in
    /// the obeyer's <c>obeys Proto[...]</c> slots). Returns null when not every obeyer parameter is
        ...
        TypeSymbol impl)
    {
        ...
        return GetOrCreateResolution(genericDef: type, typeArguments: typeArgs);
    }
