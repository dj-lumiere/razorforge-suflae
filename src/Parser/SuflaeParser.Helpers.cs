...
public partial class SuflaeParser
{
    ...
    /// </summary>
    private Expression CreateBinaryExpression(Expression left, BinaryOperator op, Expression right, SourceLocation location)
    {
        // Try to desugar to method call for overloadable operators
        Expression? desugared = TryDesugarBinaryOperator(left: left, op: op, right: right, location: location);
        if (desugared != null)
        {
            return desugared;
        }
        ...
        return new BinaryExpression(Left: left, Operator: op, Right: right, Location: location);
    }
    ...
    private Expression CreateUnaryExpression(UnaryOperator op, Expression operand, SourceLocation location)
    {
        ...
        return new UnaryExpression(Operator: op, Operand: operand, Location: location);
    }
