...
/// Detects methods that can relocate container buffers (<c>Hijacked[Byte]</c>).
///
/// Base case: A method is migratable if it can relocate the <c>Hijacked[Byte]</c> buffer
...
public sealed class MigratableInference
{
    ...
    {
        // Phase 1: Detect direct Hijacked<U8> modifications
    ...
    /// <summary>
    /// Phase 1: Detects methods that directly modify <c>Hijacked&lt;U8&gt;</c>.
    /// </summary>
    /// <remarks>
    /// A method is migratable if it reassigns the internal <c>Hijacked&lt;U8&gt;</c> member variable of an entity.
    /// All entity types use <c>Hijacked&lt;U8&gt;</c> internally for their dynamic storage.
    ...
    private void DetectDirectMigrations()
    {
        foreach (CallGraphNode node in _callGraph.AllNodes)
        {
            ...
            // 1. Operates on an entity type (all entities have internal Hijacked<U8>)
            // 2. Contains assignments that modify the Hijacked<U8> member variable
            //    (pointer, size, or capacity changes that could trigger reallocation)
            //
            // This requires analyzing the method body for Hijacked<U8> member variable writes.
