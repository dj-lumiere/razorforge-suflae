    ...
    private TypeSymbol AnalyzeBinaryExpression(BinaryExpression binary)
    {
        ...
        // Validate RHS type against the operator method's parameter type
        {
            ...
            if (methodName != null)
            {
                ...
                bool isIntegerCheckedOp = method is { IsFailable: true } &&
                    leftType is RecordTypeInfo { HasDirectBackendType: true } ltRec &&
                    ltRec.LlvmType is { } ltIr &&
                    ltIr.StartsWith(value: "i", comparisonType: StringComparison.Ordinal) &&
                    ltIr != "i1";
