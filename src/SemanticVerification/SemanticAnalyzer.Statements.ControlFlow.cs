...
public sealed partial class SemanticAnalyzer
{
    ...
    private void AnalyzeThrowStatement(ThrowStatement throwStmt)
    {
        ...
        {
            ReportWarning(code: SemanticWarningCode.ThrowAbsentInNonFailable,
                message: "Throw statement in a non-failable routine — add '!' suffix to signal callers and enable safe variant generation.",
                location: throwStmt.Location);
        }
        ...
        bool isCrashable = errorType.Category == TypeCategory.Crashable ||
                           ExplicitlyImplementsProtocol(type: errorType, protocolName: "Crashable");
        if (!isCrashable)
        {
        ...
        if (errorType.Category != TypeCategory.Record &&
            ...
            errorType.Category != TypeCategory.Crashable &&
    ...
    private void AnalyzeAbsentStatement(AbsentStatement absent)
    {
        ...
        {
            ReportWarning(code: SemanticWarningCode.ThrowAbsentInNonFailable,
                message: "Absent statement in a non-failable routine — add '!' suffix to signal callers and enable safe variant generation.",
