...
public class StackTraceCodeGen
{
    ...
    public void EmitGlobalDeclarations()
    {
        ...
        _output.AppendLine(
            value:
            "; __rf_init_symbol_tables(file_table, file_count, routine_table, routine_count, type_table, type_count)");
        _output.AppendLine(
            value: "declare void @__rf_init_symbol_tables(i8**, i32, i8**, i32, i8**, i32)");
        _output.AppendLine();
    }
    ...
    public void EmitSymbolTables()
    {
        ...
        for (int i = 0; i < _symbolTables.FileCount; i++)
        {
            ...
            _output.AppendLine(
                value:
                $"@__rf_file_{i} = private unnamed_addr constant [{len} x i8] c\"{escaped}\\00\", align 1");
        }
        ...
        for (int i = 0; i < _symbolTables.RoutineCount; i++)
        {
            ...
            _output.AppendLine(
                value:
                $"@__rf_routine_{i} = private unnamed_addr constant [{len} x i8] c\"{escaped}\\00\", align 1");
        }
        ...
        if (_symbolTables.FileCount > 0)
        {
            ...
            for (int i = 0; i < _symbolTables.FileCount; i++)
            {
                ...
                fileRefs.Add(
                    item:
                    $"i8* getelementptr inbounds ([{len} x i8], [{len} x i8]* @__rf_file_{i}, i32 0, i32 0)");
            }

            _output.AppendLine(
                value:
                $"@__rf_file_table = private constant [{_symbolTables.FileCount} x i8*] [{string.Join(separator: ", ", values: fileRefs)}], align 8");
        }
        ...
        if (_symbolTables.RoutineCount > 0)
        {
            ...
            for (int i = 0; i < _symbolTables.RoutineCount; i++)
            {
                ...
                routineRefs.Add(
                    item:
                    $"i8* getelementptr inbounds ([{len} x i8], [{len} x i8]* @__rf_routine_{i}, i32 0, i32 0)");
            }

            _output.AppendLine(
                value:
                $"@__rf_routine_table = private constant [{_symbolTables.RoutineCount} x i8*] [{string.Join(separator: ", ", values: routineRefs)}], align 8");
        }
        ...
        for (int i = 0; i < _symbolTables.TypeCount; i++)
        {
            ...
            _output.AppendLine(
                value:
                $"@__rf_type_{i} = private unnamed_addr constant [{len} x i8] c\"{escaped}\\00\", align 1");
        }
        ...
        if (_symbolTables.TypeCount > 0)
        {
            ...
            for (int i = 0; i < _symbolTables.TypeCount; i++)
            {
                ...
                typeRefs.Add(
                    item:
                    $"i8* getelementptr inbounds ([{len} x i8], [{len} x i8]* @__rf_type_{i}, i32 0, i32 0)");
            }

            _output.AppendLine(
                value:
                $"@__rf_type_table = private constant [{_symbolTables.TypeCount} x i8*] [{string.Join(separator: ", ", values: typeRefs)}], align 8");
        }
    ...
    public void EmitSymbolTableInitFunction()
    {
        ...
        {
            fileTablePtr =
                $"bitcast ([{_symbolTables.FileCount} x i8*]* @__rf_file_table to i8**)";
        }
        else
        {
            fileTablePtr = "null";
        ...
        {
            routineTablePtr =
                $"bitcast ([{_symbolTables.RoutineCount} x i8*]* @__rf_routine_table to i8**)";
        }
        else
        {
        ...
        {
            typeTablePtr =
                $"bitcast ([{_symbolTables.TypeCount} x i8*]* @__rf_type_table to i8**)";
        }
        else
        {
            typeTablePtr = "null";
        ...
        _output.AppendLine(
            value:
            $"  call void @__rf_init_symbol_tables(i8** {fileTablePtr}, i32 {_symbolTables.FileCount}, i8** {routineTablePtr}, i32 {_symbolTables.RoutineCount}, i8** {typeTablePtr}, i32 {_symbolTables.TypeCount})");
        ...
        _output.AppendLine(
            value:
            "@llvm.global_ctors = appending global [1 x { i32, void ()*, i8* }] [{ i32, void ()*, i8* } { i32 65535, void ()* @__rf_init_symbols, i8* null }]");
        _output.AppendLine();
    }
    ...
    public void EmitPushFrame(uint fileId, uint routineId, uint typeId, uint line, uint column)
    {
        ...
        _output.AppendLine(
            value:
            $"  call void @__rf_stack_push(i32 {fileId}, i32 {routineId}, i32 {typeId}, i32 {line}, i32 {column})");
    }
    ...
    public void EmitThrow(string errorTypePtr, string messagePtr)
    {
        if (!Enabled)
        {
            ...
            _output.AppendLine(
                value: $"  call void @__rf_throw(i8* {errorTypePtr}, i8* {messagePtr})");
