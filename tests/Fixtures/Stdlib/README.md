...
- **Deterministic output only.** For Set/SecureSet/Dict/SecureDict (non-deterministic iteration order), probe via:
...
- Containers: `list`, `set`, `dict`, `deque`, `tuple`, `bitlist`, `sorted_list`, `sorted_set`, `sorted_dict`, `secure_set`, `secure_dict`, `array`, `bitarray`
